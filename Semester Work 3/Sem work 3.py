...
#L = 100
#M = 75
#SimulatedDataset = pd.DataFrame(np.random.gamma(shape,scale,size=(30,1000)))
#simulatedTraj = pd.DataFrame(SimulatedDataset.cumsum())
#time = np.linspace(0,500,30)
#plt.plot(time,simulatedTraj, color = "lightsteelblue")
...
#
#for column in simulatedTraj:
#    columnVal = simulatedTraj[column].values
#    for element in range(0,len(columnVal)):
#        if columnVal[element] > 100: 
#            TimeofThresholdPassing.append(time[element])
#            break
#print(len(TimeofThresholdPassing))
##plt.hist(TimeofThresholdPassing, bins=50, histtype='bar')
##plt.xlabel('Time')
##plt.ylabel('Frequency')
